...
function continueQuickBrief() {
  ...
  const deliveryDate = $('quickDeliveryDeadline')?.value || '';
  const today = getLocalTodayIso();
  if (!lastCalc) {
    $('quickBriefStatus').textContent = 'กรุณาตรวจสอบขนาด จำนวน และ Preset กระดาษ';
    return;
  }
  if (!deliveryDate || deliveryDate < today) {
    $('quickBriefStatus').textContent = 'กรุณาเลือกวันที่ต้องการรับงานตั้งแต่วันนี้เป็นต้นไป';
    $('quickDeliveryDeadline')?.focus();
    return;
  }
  $('jobName').value = name;
  $('jobName').dataset.autoGenerated = name;
  ...
  if (deliveryNative) deliveryNative.value = deliveryDate;
  $('briefFileLink').value = $('quickBriefFileLink')?.value.trim() || '';
