...
assert.equal(PDF_MAX_BYTES, 10 * 1024 * 1024);
...
await assert.rejects(pdfToPng({ size: PDF_MAX_BYTES + 1, name: 'big.pdf' }), /ใหญ่เกิน 10 MB/);
...
assert.match(html, /รับ SVG \(ไม่เกิน 5 MB\), PDF, PNG, JPG, WebP \(ไม่เกิน 10 MB\)/);
