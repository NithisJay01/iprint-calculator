...
assert.equal(calculatorContext.normalizePreviewGuidelineOpacity(undefined), 30);
assert.match(quickBriefHtml, /id="previewGuidelineOpacity"[^>]+min="0"[^>]+max="100"[^>]+step="5"[^>]+value="30"/);
assert.match(quickBriefHtml, /js\/brief\.js\?v=20260911-single-brief/);
assert.match(quickBriefHtml, /js\/artwork\.js\?v=20260911-single-brief/);
assert.match(briefSource, /kind:'brief'[\s\S]+?ภาพบรีฟงานพิมพ์/);
assert.match(briefSource, /kind:'reference'[\s\S]+?ภาพ Ref/);
assert.match(briefSource, /งานที่ตัดแล้ว/);
assert.match(briefSource, /viewBox="\$\{bleed\} \$\{bleed\} \$\{trimW\} \$\{trimH\}"/);
