...
stored.properties['Ticket URL'] = { url: 'https://notion.test/ticket-1' };
assert.equal((await repository.listByTicketId('ticket-1')).length, 1, 'ticket URL should be used when Notion omits relation values');
stored.properties['Order Ticket'] = storedRelation;
stored.properties['Ticket URL'] = storedTicketUrl;
