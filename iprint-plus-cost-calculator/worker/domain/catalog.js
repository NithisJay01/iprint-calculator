...
export function normalizeCatalogItem(input = {}) {
  ...
  return {
    ...
    material3dKey: String(input.material3dKey || '').trim(),
    materialFamily: String(input.materialFamily || '').trim(),
    specValue: input.specValue != null && Number.isFinite(Number(input.specValue)) ? Number(input.specValue) : null,
    specUnit: String(input.specUnit || '').trim(),
